...
public class TimeConfirmation {
	...
	public ArrayList<ArrayList<String>> getTime(String month, Student student) throws FileNotFoundException, NullPointerException {
		...
		//JDBCドライバを明示的にロード
			...
			try(Connection conn = DriverManager.getConnection(url, user, pass)){
				...
				//String型の引数monthをint型に変換
				//もし1〜9月なら前に0をつけたString型に変換
				//10〜12月ならそのままString型に戻す
				int month_int = Integer.parseInt(month);
				String month_string;
				if(month_int < 	10) {
					month_string = "0" + month_int;
				}else {
					month_string = String.valueOf(month_int);
				}
			
				//引数のmonthを渡して、DBのwork_tbからdayカラムにmonthを含むレコードのデータを取得するSQL文
				String getTime_sql = "select attendance_time, attendance, leaving_time, leaving from work_tb where student_id = " + student.getId() + " and attendance_time like '____-" + month_string + "%'";
